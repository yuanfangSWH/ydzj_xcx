...
Page({
  ...
  loadAdvertising: function () {
    ...
    if (advertisingData == '' || !advertisingData) {
      app.advertising((code, data) => {
        if (code == 0) {
          this.setData({
            advertisingImag: data.shop_list_top ? data.shop_list_top : []
          })
        }
      });
    } else {
      this.setData({
        advertisingImag: advertisingData.shop_list_top ? advertisingData.shop_list_top : []
      })
    }
  },
  //Banner点击
  BannerBindtap: function () {
    ...
    app.JAnalyticsInterface.onEvent({
      ...
      attributes: {
        event_id: 'ShoplistTopBannerAdClickld'
      }
    })
  },
  //商家详情
